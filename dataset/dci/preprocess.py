...
def create_dataset(preprocessor, dataset, output_dir, cn_type='canny', enable_no_prompt=False, enable_extra_caption=True):
    """
    ...
    origin_data = list_matching_file_paths_regex(os.path.join(dataset, 'annotations'), r".*\.json")
    
    total_samples = len(origin_data)
    print(f"Processing {total_samples} samples for ControlNet {cn_type} dataset...")
    
    for i, orig in enumerate(tqdm(origin_data, total=total_samples, desc="Processing samples"), 1):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", type=str, default="densely_captioned_images",
