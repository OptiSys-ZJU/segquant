...
from backend.torch.layers.embeddings import ImageProjection
...
class StableDiffusionXLModel(nn.Module):
    ...
    def from_repo(
        cls,
        repo: str = "stable-diffusion-xl-base-1.0",
        device="cuda:0",
        unet=None,
    ):
        main_repo = repo
        ...
        if unet is None:
            unet = (
                UNet2DConditionModel.from_config(
                    f"{main_repo}/unet/config.json",
                    f"{main_repo}/unet/diffusion_pytorch_model.fp16.safetensors",
                )
                .half()
                .to(device)
            )
        ...
        vae = (
            AutoencoderKL.from_config(
                ...
                f"{main_repo}/vae/diffusion_pytorch_model.fp16.safetensors",
            )
            .half()
            .to(device)
        )
