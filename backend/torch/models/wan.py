...
from backend.torch.modules.autoencoder_kl_wan import AutoencoderKLWan
from backend.torch.modules.transformer_wan import WanTransformer3DModel
...
from backend.torch.utils.video_processor import VideoProcessor
...
class WanModel(nn.Module):
    ...
    def from_repo(
        cls,
        repo: str = "Wan2.1-T2V-1.3B-Diffusers",
        device="cuda:0",
        transformer=None,
    ):
        main_repo = repo
        ...
        if transformer is None:
            transformer = (
                WanTransformer3DModel.from_config(
                    ...
                    f"{main_repo}/transformer/diffusion_pytorch_model.safetensors.index.json",
                )
                .half()
                .to(device)
            )
        ...
        tokenizer = AutoTokenizer.from_pretrained(f"{main_repo}/tokenizer")
