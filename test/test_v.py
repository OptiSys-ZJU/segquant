...
if __name__ == "__main__":
    ...
    def compute_k_matrix(xs):
        ...
        k_flat = theta[0]
        k_matrix = k_flat.view(*shape)
        return k_matrix

    real_perfect_v = compute_k_matrix(real_latents)
    quant_perfect_v = compute_k_matrix(quant_latents)

    def compute_signed_angle_matrix(
        real_ks: torch.Tensor, quant_ks: torch.Tensor, eps=1e-8
    ):
        k1 = real_ks
        k2 = quant_ks
        ...
        theta_rad = torch.atan(numerator / (denominator + eps))
    ...
    def visualize_k_heatmaps_4x4(
        real_ks: torch.Tensor, quant_ks: torch.Tensor, save_path="k_heatmaps_4x4.png"
    ):
        assert real_ks.shape == quant_ks.shape
        assert real_ks.shape[1] == 16
    ...
    def plot_pos_neg_heatmap(
        angle_tensor: torch.Tensor, save_path="pos_neg_heatmap.png"
    ):
    ...
    def channelwise_corr_and_fit(real_tensor: torch.Tensor, quant_tensor: torch.Tensor):
        ...
        assert (
            real_tensor.dim() == 4 and real_tensor.shape[0] == 1
    ...
    corrs, a, b = channelwise_corr_and_fit(
        affiner.latent_record[1][-1][1], real_perfect_v
    )

    for i in range(len(corrs)):
        print(
            f"Channel {i:2d} | Corr: {corrs[i]:.4f} | y = {a[i]:.4f} * x + {b[i]:.4f}"
        )
