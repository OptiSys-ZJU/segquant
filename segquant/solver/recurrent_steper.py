...
class RecurrentSteper(BaseSteper):
    def __init__(self, max_timestep, sample_size=1, solver_type=BaseSolver, solver_config=None, latents=None, recurrent=True, noise_target='all', enable_latent_affine=False, enable_timesteps=None, device='cuda:0'):
        ...
        if self.enable_latent_affine and (i in self.enable_timesteps):
            if self.fsm.state == Stage.FINAL:
                ...
                if self.latent_diff[i][1] == 0:
                    ...
                    self.latent_diff[i] = (self.latent_diff[i][0] + this_diff, self.latent_diff[i][1] + 1)
