...
def load_real_quant_mix_ext(verbose=False, required=False):
    ...
    def Wfp16Afp16_real_quantized_gemm_scaled(self, input, weight, scale_x, scale_w):
        s_x = input.new_tensor([1.0])
        s_w = input.new_tensor([1.0])
        return self.prototype_Wfp16Afp16_real_quantized_gemm_scaled(input, weight, s_x, s_w)
    
    def Wfp16Afp16_real_quantized_gemm_dual_scaled(self, input, weight, pos_scale_x, neg_scale_x, scale_w):
        s_px = input.new_tensor([1.0])
        s_nx = input.new_tensor([1.0])
        s_w = input.new_tensor([1.0])
        return self.prototype_Wfp16Afp16_real_quantized_gemm_dual_scaled(input, weight, s_px, s_nx, s_w)
