...
def infer(prefix, model, type, num_inference_steps=1, scale=0.5, latents=None):
    ...
    image[0].save(f'{prefix}-{type}-{num_inference_steps}-{scale}.jpg')
    print("=============================")
    return image[0]
...
if __name__ == '__main__':
    ...
    #######################
    # infer('int8_default', sd3, 'canny', 1, 0.8, torch.load('latents.pt'))
    latents = torch.load('latents.pt')
    for scale in [0.2, 0.5, 0.8]:
        for i in range(1, 29, 3):
            infer('int8_smoothquant', sd3, 'canny', i, scale, latents)
